...
int save_freq(double freq)
{
        ...
        if (freq < 76.0  || freq > 200.0) return -1;
...
void freq_scan(int mode, int forced_mono)
{
        ...
        do {
                ...
                count++;
        } while(freq < 180.0 && count < MAX_STATION);
...
int main(int argc, char *argv[])
{
        ...
        if (!strncmp(argv[1], "scan", len)) {
                ...
                freq = get_freq() + 0.1;
                if (freq < 76.0 || freq > 180.0) exit(2);
                ...
                if ((freq >= 76.0 && freq <= 180.0) || (preset >= 0 && preset < station_info_num)) {
                        if (freq >= 76.0 && freq <= 180.0) freq = strtod(argv[1], NULL);
